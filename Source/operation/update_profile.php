...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      $select_name = $conn->prepare("SELECT * FROM `operation` WHERE name = ?");
      $select_name->execute([$name]);
      if($select_name->rowCount() > 0){
         $message[] = 'username already taken!';
      }else{
         $update_name = $conn->prepare("UPDATE `operation` SET name = ? WHERE id = ?");
         $update_name->execute([$name, $op_id]);
      }
   }

   $empty_pass = 'da39a3ee5e6b4b0d3255bfef95601890afd80709';
   $select_old_pass = $conn->prepare("SELECT password FROM `operation` WHERE id = ?");
   $select_old_pass->execute([$op_id]);
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         if($new_pass != $empty_pass){
            $update_pass = $conn->prepare("UPDATE `operation` SET password = ? WHERE id = ?");
            $update_pass->execute([$confirm_pass, $op_id]);
...
<section class="form-container">
   ...
   <form action="" method="POST">
      <h3>update operator profile</h3>
      <input hidden type="text" name="name" maxlength="20" class="box" oninput="this.value = this.value.replace(/\s/g, '')" placeholder="<?= $fetch_profile['name']; ?>">
